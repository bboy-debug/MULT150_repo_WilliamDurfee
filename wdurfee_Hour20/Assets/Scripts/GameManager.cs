...
public class GameManager : MonoBehaviour
{
    ...
    void Update() // <--Keeps track of time.
    {
        if (isGameOver)
            return;
    ...
    public void AdjustTime(float amount) //<-- Whenever a player hits an obstacle or a power-up.
    {
        gameTime += amount;
        if (amount < 0)
            SlowWorldDown();
    ...
    void SlowWorldDown() //<-- Whenever a player hits an obstacle (pg369).
    {
    ...
    void OnGUI() //<--draws the remaiining time to the scene while the game is running, and it shows the total time the game lasted after it ends.
    {
        if (!isGameOver)
        {
